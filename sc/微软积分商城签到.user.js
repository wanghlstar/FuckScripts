            ...
            msg: {
                ...
                "markdown": {
                    ...
                    get text() {
                        const account = GM_getValue("Notice.account", "账号A");
                        // 处理 sendMSG 中的换行
                        let msgContent = FuckD.bing.sendMSG;                
                        // 方法1：将单换行替换为双换行（推荐）
...
FuckF.taskPromos = async () => {
    ...
    const requestToken = 1  // await FuckF.getRewardsToken() //20260222 fix
...
FuckF.taskSearch = async () => {
    ...
    if (FuckD.search.times > 2 || FuckD.search.index >= FuckD.search.limit) {
        ...
        // 每轮搜索完成推送，最后不到随机次数，按所有搜索任务完成了推送一次
        if (!GM_getValue("Notice.allpush", true)) {
            FuckF.send(FuckD.wh)
        }
        return true
    }
